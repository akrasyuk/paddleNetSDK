...
public class DefaultTaxCategoryConverter : JsonConverter<TaxCategory>
{
...
public class NullableTaxCategoryConverter : JsonConverter<TaxCategory?>
{
    public override TaxCategory? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        var stringValue = reader.GetString();
...
public static class TaxCategoryConverter
{
    public static ICollection<JsonConverter> GetConverters() =>
        [new DefaultTaxCategoryConverter(), new NullableTaxCategoryConverter()];
}